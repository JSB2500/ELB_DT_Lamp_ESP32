...
void WifiServer_Go()
{
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      ssize_t NumBytesRead = recv(ClientSocket, &CharRead, 1, 0);

      if (NumBytesRead < 0)
      {
        ESP_LOGE(WiFiLogTag, "Received: %d %s", NumBytesRead, strerror(errno));
        break;
      }
      ...
      if (CharRead == '\n')
      {
        if (InputBuffer_NumBytes == 0) // See whether previous line was blank too i.e. see whether we've received two consecutive blank lines.
        {
        ...
        // Do it here rather than at the end of the request to avoid storing the whole request.

        pInputBuffer[InputBuffer_NumBytes] ='\0';
        ESP_LOGI(WiFiLogTag, "Request: %s", pInputBuffer);

        // NB: Shared variables are not protected from simultaneous access!
        if (FirstLine) // Only check first line of request.
        {
          if (strncmp(pInputBuffer, "GET /", 5) == 0)
          {
            ...
            if (strncasecmp(pCommand, "State?", 6) == 0)
            {
              ...
              char *pEquals;
              pEquals = strpbrk(pCommand, "=");
              if (pEquals)
                *pEquals = ' ';

              int32_t NumberOfItems = sscanf(pCommand, "%32s %f", ParameterName, &ParameterValue);
              if (NumberOfItems == 2)
              {
                if (ParameterValue < 0.0f)
                 ...
                 if (strcasecmp(ParameterName, "N") == 0)
                   NaturalBrightness = ParameterValue;
                 else if (strcasecmp(ParameterName, "W") == 0)
                   WarmBrightness = ParameterValue;
                 else if (strcasecmp(ParameterName, "R") == 0)
                   RedBrightness = ParameterValue;
                 else if (strcasecmp(ParameterName, "G") == 0)
                   GreenBrightness = ParameterValue;
                 else if (strcasecmp(ParameterName, "B") == 0)
                   BlueBrightness = ParameterValue;
              }
            }
            else
            {
               if (strncasecmp(pCommand, "Off ", 4) == 0)
               {
                 Off = 1;
                 OffChanged = 1;
               }
               else if (strncasecmp(pCommand, "On ", 3) == 0)
               {
                  Off = 0;
                  OffChanged = 1;
                }
                else if (strncasecmp(pCommand, "Night ", 6) == 0)
               {
                 NaturalBrightness = 0.0f;
                 WarmBrightness = 0.3f;
               }
               else if (strncasecmp(pCommand, "Bright ", 7) == 0)
               {
        ...
        InputBuffer_NumBytes = 0; // Clear input buffer.

        FirstLine = 0;
      }
      else if (CharRead != '\r')
      {
        if (InputBuffer_NumBytes < InputBuffer_SizeInBytes - 1)
        {
          pInputBuffer[InputBuffer_NumBytes] = CharRead;
          ++InputBuffer_NumBytes;
        }
      }
    }
...
static void Go()
{
  ...
  ESP_LOGV(DefaultLogTag, "Go: Begin");
  ...
  ESP_LOGV(DefaultLogTag, "Go: Eternal loop: Begin");
  ...
  while (1)
  {
    if (XPT2046_Sample(&Touch_RawX, &Touch_RawY, &Touch_RawZ))
    {
      ...
      ESP_LOGV(DefaultLogTag, "Go: ProcessTouch: Begin");
	  ProcessTouch(Touch_X, Touch_Y);
	  ESP_LOGV(DefaultLogTag, "Go: ProcessTouch: End");
    }
    else
    {
      ButtonPressed = 0;
